//! Typed view over the frozen provider registry.
//!
//! The data in `data/registry.json` is generated by `tools/extract-models-fetcher.py`
//! from a reference dump, not written by hand, so provider transports, auth
//! descriptors, and model tables stay byte-faithful to the source they came from.
