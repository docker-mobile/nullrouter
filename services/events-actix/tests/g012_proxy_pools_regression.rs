...
fn parse_event_names(body: &str) -> TestResult<Vec<String>> {
    ...
    for raw_frame in body.split("\n\n").filter(|frame| !frame.trim().is_empty()) {
        ...
        serde_json::from_str::<Value>(&data).map_err(|error| {
            test_error(format!(
                "invalid G012 SSE JSON data for {event}: {error}; body: {body}"
            ))
        })?;
        events.push(event);
    }
...
fn assert_markup_free_event_stream(body: &str) {
    ...
    assert!(
        ...
        "G012 event-stream response contained an HTML doctype: {body}"
    );
    assert!(
        !normalized.contains("<html"),
        "G012 event-stream response contained an HTML document tag: {body}"
    );
}

#[actix_web::test]
async fn g012_event_stream_routes_remain_parseable_while_proxy_pool_routes_change() -> TestResult {
    // Given: proxy-pool route work must not perturb existing browser-facing SSE surfaces.
    let probes = [
        G012StreamProbe {
    ...
    // When: each events route is requested as a text/event-stream endpoint.
    for probe in probes {
        let response = get_stream(probe.uri).await?;
        let events = parse_event_names(&response.body)?;

        // Then: each route returns parseable JSON SSE frames without an HTML fallback.
        ...
        for expected_event in probe.expected_events {
            assert!(
                ...
                "G012 missing SSE event {expected_event} in {}: {events:?}",
                probe.uri
            );
        }
    }
