...
const PROXY_POOL_API_ROUTES: [&str; 6] = [
    "/api/proxy-pools",
    "/api/proxy-pools/{id}",
...
#[actix_web::test]
async fn route_catalog_includes_g012_proxy_pool_ownership_as_json_when_requested() -> TestResult {
    // Given: Proxy Pools parity needs dashboard and API route ownership in catalog JSON.
    ...
    assert_routes_include(api_routes, &["/api/dashboard/chat/completions"]);
    assert_routes_include(api_routes, &PROXY_POOL_API_ROUTES);
