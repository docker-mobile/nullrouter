...
#[actix_web::test]
async fn translator_dashboard_serves_wasm_shell_when_requested() -> TestResult {
    ...
    // When: the Translator dashboard route is requested directly.
    let html = dashboard_shell_for!(app, "/dashboard/translator");
...
#[actix_web::test]
async fn translator_api_routes_are_not_dashboard_pages_when_requested() -> TestResult {
    ...
    for (method, path, payload) in [
        ...
        (
            Method::POST,
            "/api/translator/send",
            r#"{"provider":"openai","model":"gpt-4o-mini","body":{}}"#,
        ),
    ] {
        // When: a Translator API path is requested against the dashboard host.
...
async fn existing_dashboard_routes_keep_serving_wasm_shell_when_requested() -> TestResult {
    ...
    for path in ["/dashboard/basic-chat", "/dashboard/proxy-pools"] {
