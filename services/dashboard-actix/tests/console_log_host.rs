...
macro_rules! dashboard_shell_for {
    ($app:expr, $path:expr) => {{
        ...
        assert!(
            html.contains("<link rel=\"preload\" href=\"/pkg/dashboard_leptos_bg.wasm\" as=\"fetch\" type=\"application/wasm\" crossorigin>"),
            "{path}"
        );
...
#[actix_web::test]
async fn console_log_dashboard_serves_wasm_shell_when_requested() -> TestResult {
    ...
    // When: the Console Log dashboard route is requested directly.
...
#[actix_web::test]
async fn console_log_api_routes_are_not_dashboard_pages_when_requested() -> TestResult {
    ...
    for path in [
        ...
        // When: a Console Log API path is requested against the dashboard host.
        let response =
            test::call_service(&app, test::TestRequest::get().uri(path).to_request()).await;
...
#[actix_web::test]
async fn neighboring_dashboard_routes_keep_serving_wasm_shell_when_requested() -> TestResult {
    // Given: the dashboard host has the built Leptos package available.
    let (_root, config) = fixture_config()?;
    let app = test::init_service(App::new().configure(config.into_configurer())).await;

    for path in ["/dashboard/translator", "/dashboard/proxy-pools"] {
        // When: a neighboring dashboard route is requested directly.
