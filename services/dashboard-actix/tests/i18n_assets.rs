...
/// The picker's bare `en` entry, which resolves to the `en-US` file below.
const UNTRANSLATED_LOCALE: &str = "en";

/// The source locale's own file, generated from the keys used in dashboard source.
const SOURCE_LOCALE_FILE: &str = "en-US";
...
fn literal_file_count_matches_the_locale_list() -> TestResult {
    ...
    // The source locale ships a file of its own: the dashboard fetches its own
    // locale map at runtime, so English is served rather than compiled in, and
    // `tools/i18n-gen` treats that file as the authority every other locale is
    // generated against.
    let mut expected = LOCALES
        .iter()
        .filter(|locale| **locale != UNTRANSLATED_LOCALE)
        .map(|locale| (*locale).to_owned())
        .chain(std::iter::once(SOURCE_LOCALE_FILE.to_owned()))
        .collect::<Vec<_>>();
    ...
    assert_eq!(
        shipped.len(),
        expected.len(),
        "expected {} literal files, got {shipped:?}",
        expected.len()
    );
...
#[test]
fn no_literal_key_is_unreachable() -> TestResult {
    // Lookup is by exact `section.key`, so a key stored with surrounding whitespace can never be
    // hit. The four such keys upstream shipped (`"Security required: "`, in its largest files)
    // are gone: every file is now generated by `tools/i18n-gen` from the keys the dashboard
    // actually looks up, so an unreachable key can only appear if the generator regresses.
    ...
    assert!(
        ...
        "generated locale files must carry no unreachable keys, got {unreachable:?}"
    );
